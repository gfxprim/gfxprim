...
#define GP_GETPIXEL_4BPP(context, x, y) ({ \
	uint8_t *gp_pix_addr = GP_PIXEL_ADDR(context, x, y); \
	(*gp_pix_addr & (0xf0 >> (4*(x%2)))) >> (4*(1 - x%2)); \
})	

#define GP_GETPIXEL_8BPP(context, x, y) ({ \
	uint8_t *gp_pix_addr = GP_PIXEL_ADDR(context, x, y); \
	gp_pix_addr[0]; \
})

#define GP_GETPIXEL_16BPP(context, x, y) ({ \
	uint8_t *gp_pix_addr = GP_PIXEL_ADDR(context, x, y); \
	gp_pix_addr[0]<<8 | gp_pix_addr[1]; \
})

#define GP_GETPIXEL_24BPP(context, x, y) ({ \
	uint8_t *gp_pix_addr = GP_PIXEL_ADDR(context, x, y); \
	gp_pix_addr[0]<<16 | gp_pix_addr[1] << 8 | \
	gp_pix_addr[2]; \
})

#define GP_GETPIXEL_32BPP(context, x, y) ({ \
	uint8_t *gp_pix_addr = GP_PIXEL_ADDR(context, x, y); \
