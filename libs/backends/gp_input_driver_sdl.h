...
void gp_input_driver_sdl_event_put(gp_backend *backend, gp_ev_queue *event_queue, SDL_Event *ev)
{
	...
	case SDL_VIDEORESIZE:
		schedule_resize(event_queue, ev->resize.w, ev->resize.h);
	break;
	...
	case SDL_WINDOWEVENT:
		...
		case SDL_WINDOWEVENT_RESIZED:
			schedule_resize(event_queue, ev->window.data1, ev->window.data2);
		break;
		case SDL_WINDOWEVENT_EXPOSED:
			gp_backend_flip(backend);
		break;
		case SDL_WINDOWEVENT_FOCUS_GAINED:
			gp_ev_queue_push_focus_in(event_queue, 0);
		break;
		case SDL_WINDOWEVENT_FOCUS_LOST:
			gp_ev_queue_push_focus_out(event_queue, 0);
		break;
		case SDL_WINDOWEVENT_SHOWN:
			render_start(backend);
		break;
