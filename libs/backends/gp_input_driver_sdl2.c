...
static uint16_t sdl2_keysym_table1[] = {
 0,                  0,                 0,                 0,
...
static uint16_t sdl2_keysym_table2[] = {
...
void gp_input_driver_sdl2_event_put(gp_event_queue *event_queue, SDL_Event *ev)
{
	...
	case SDL_MOUSEMOTION:
		gp_event_queue_push_rel(event_queue, ev->motion.xrel, ev->motion.yrel, NULL);
	break;
	...
	case SDL_MOUSEBUTTONUP:
		...
		case 5:
			...
			return;
		default:
			GP_WARN("Unmapped SDL Mouse button %02x", ev->button.button);
			return;
		}
	...
	case SDL_KEYUP:
		...
		if (keysym > 0 && keysym <= GP_ARRAY_SIZE(sdl2_keysym_table1))
			key = sdl2_keysym_table1[keysym - 1];

		if (keysym > 255 && keysym <= 255 + GP_ARRAY_SIZE(sdl2_keysym_table2))
			key = sdl2_keysym_table2[keysym - 256];
	...
	case SDL_WINDOWEVENT:
		if (ev->window.event == SDL_WINDOWEVENT_SIZE_CHANGED || ev->window.event == SDL_WINDOWEVENT_RESIZED) {
			gp_event_queue_push_resize(event_queue, ev->window.data1, ev->window.data2, NULL);
        }
	break;
	case SDL_QUIT:
		gp_event_queue_push(event_queue, GP_EV_SYS, GP_EV_SYS_QUIT, 0, NULL);
	break;
	}
}
