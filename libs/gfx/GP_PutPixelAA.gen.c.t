                       ...
                       GP_Pixel pixel)
{
	...
	GP_MixPixel_Raw_Clipped(pixmap, int_x, int_y, pixel, perc);

	perc = FP_TO_PERC(GP_FP_MUL(frac_x, GP_FP_1 - frac_y));
	GP_MixPixel_Raw_Clipped(pixmap, int_x + 1, int_y, pixel, perc);

	perc = FP_TO_PERC(GP_FP_MUL(GP_FP_1 - frac_x, frac_y));
	GP_MixPixel_Raw_Clipped(pixmap, int_x, int_y + 1, pixel, perc);

	perc = FP_TO_PERC(GP_FP_MUL(frac_x, frac_y));
	GP_MixPixel_Raw_Clipped(pixmap, int_x + 1, int_y + 1, pixel, perc);
}

void GP_PutPixelAA_Raw_Clipped(GP_Pixmap *pixmap, GP_Coord x, GP_Coord y,
                               GP_Pixel pixel)
{
	GP_PutPixelAA_Raw(pixmap, x, y, pixel);
}
...
void GP_PutPixelAA(GP_Pixmap *pixmap, GP_Coord x, GP_Coord y, GP_Pixel pixel)
{
	...
	GP_PutPixelAA_Raw_Clipped(pixmap, x, y, pixel);
}
